...
- **BD1ドメイン残基範囲(44–168)**: 現時点では既存文献等を参考にした決め打ちの範囲。構造解析パイプライン（BSA・保存度スコア計算）を用いて、この範囲選定の妥当性を客観的な基準で裏付けることを今後の課題としている（下記参照）。
...
- **アプリ化**: Streamlit等を用いた簡易UIの実装。SMILESを入力して予測pIC50を確認できる形を想定。
- **タンパク質×タンパク質（バイオ医薬）版の検討**: 私の構造生物学の専門性との接続を狙い、SKEMPIデータセットを用いたΔΔG予測など、既存のGNNコードベースを流用した軽量版の実施を検討中。
